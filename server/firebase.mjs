...
if (!GOOGLE_APPLICATION_CREDENTIALS) throw 'No GOOGLE_APPLICATION_CREDENTIALS found in environment variables';
...
export async function authenticateToken(firebase_token) {
	...
	return {
		...
		rmtID: await getRMTIDFromFirebaseID(user.user_id),
		admin: await getAdminFromFirebaseID(user.user_id),
	};
}
